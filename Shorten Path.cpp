...
A path can be an absolute path, meaning that it starts at the root directory in a file system, or a relative path, meaning that it starts at the current directory 
in a file system.
...
- The symbol / otherwise represents the directory separator. This means that the path /foo/bar is the location of the directory bar inside the directory foo, 
  which is itself located inside the root directory.
...
- The symbols / and . can be repeated sequentially without consequence; the symbol .. cannot, however, because repeating it sequentially means going further up in parent 
  directories. For example, /foo/bar/baz/././. and /foo/bar/baz are equivalent paths, but /foo/bar/baz/../../../ and /foo/bar/baz definitely aren't. The only exception 
  is with the root directory: /../../.. and / are equivalent, because the root directory has no parent directory, which means that repeatedly accessing parent directories 
  does nothing.
...
string shortenPath(string path) {
  stack<string> s;
	...
	{
		if(path[i] == '/' 
			 || (i == path.length() - 1 && path[i] == '.') 
			 || (i < path.length() - 1 && path[i] == '.' && path[i + 1] != '.')) 
			continue;
		int cur = i;
		while(i < path.length() && path[i] != '/') i++;
		i--;
		string samp = path.substr(cur, i - cur + 1);
		if(samp == ".." && flag == 1){
			if(s.size() != 0)
				s.pop();
			i++;
  ...
  return path[0] == '/' ? fn : fn.substr(1, fn.size() - 1);
}
